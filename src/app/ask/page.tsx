      ...
      <section className="card mx-auto max-w-2xl p-6 md:p-8">
        ...
        {!session ? (
          <div className="mt-6 rounded-2xl border border-rose-200 bg-rose-50 p-4 text-sm text-rose-700">
            Please <Link href="/login" className="text-sky-600 underline">sign in</Link> to ask a question.
          </div>
        ) : (
          <form onSubmit={handleSubmit} className="mt-6 space-y-5">
            ...
            {questionType === 'multiple_choice' ? <div className="space-y-4">
              ...
              {choices.map((choice, index) => (
                <div key={index} className="flex items-center gap-3">
                  <label className="flex items-center gap-2">
                    ...
                    />
                    <span className="text-slate-700">{String.fromCharCode(65 + index)}</span>
                  </label>
                  <input
                    type="text"
                    value={choice}
                    onChange={(event) => handleChoiceChange(index, event.target.value)}
                    className="flex-1 rounded-2xl border border-slate-300 bg-white px-4 py-3 focus:border-sky-500 focus:outline-none"
              ...
              <label className="block">
                ...
                />
                <span className="mt-1 block text-xs text-slate-500">Separate accepted aliases with commas. There is also some inbuilt smart matching that will on a best effort accommodate things like abbreviations, minor typos, punctuation, etc.</span>
              </label>
            )}
